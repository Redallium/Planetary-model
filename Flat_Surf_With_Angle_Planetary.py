#Программа для рассчета распределения толщины покрытия на плоской детали, наклонённой к горизонтали под углом при покрытии с использованием планетарной модели
import math
sqrt = math.sqrt
sin = math.sin
cos = math.cos
...
try:
	F = open("Graph_Flat_Surf_With_Angle_Planetary.txt", "w")
except IOError:
...
for n in range(N):       #Цикл по всем контрольным точкам
	...
	for t in TimeMoments:                  #Цикл по всем моментам времени
		dx = d + r * cos(w2 * t) * cos(phi) - L * cos(w1 * t)     #Разница x - овых координат n - ой контрольной точки и испарителя
		dy = r * sin(w2 * t) - L * sin(w1 * t)	 			 			#Разница y - овых координат n - ой контрольной точки и испарителя
		dz = H - r * cos(w2 * t) * sin(phi)	 				    #Разница z - овых координат n - ой контрольной точки и испарителя.
		l = sqrt(dx * dx + dy * dy + dz * dz)    			 			#Формула для расстояния между испарителем и контрольной точкой n
		dh = (((dz / l) ** alpha) * (sqrt(1 - (dz * dz) / (l * l)) * sin(phi) + (dz / l) * cos(phi))) / (l * l)   #Формула для наростающей толщины (пропорциональна косинусу угла вылета из испарителя и косинусу угла падения на деталь и обратно пропорцилнальна квадрату расстояния от испарителя до контрольной точки)
